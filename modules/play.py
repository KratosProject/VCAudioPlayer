...
@Client.on_message(filters.command(['play', 'play@vcaudio_bot']) & ~filters.edited & ~filters.bot)
@chat_allowed
@admin_mode_check
                ...
                if filename is None:
                    ...
                    if Config.get('PLAYBACK_FOOTER') not in ['', None]:
                        footer = f"{Config.get('PLAYBACK_FOOTER')}".replace(
                            '\\n', '\n')
                    footer_val = (
                        '\n'+footer) if footer is not None else '\nFor any issues contact @botsupportgr'
                    ...
                    if response is not True:
                        ...
                        if cover_file_name is not None and os.path.exists(cover_file_name):
                            ...
                            caption = f"**🎧 Name:** `{(song_info['title'].strip())[:20]}`\n**⏱ Duration:** `{song_info['duration']}`\n**💡 Requester:** {req_by}\n\n`Join voice chat to listen to the song.`@botsupportgr"
